...
| `scripts/fetch_icons.sh` | AWS pack + the original product marks. Copy into the project's `icons/` and edit the lists: Simple Icons by slug, project marks by URL, the AWS pack by path (pass the release URL as `$1`). Every icon the figure uses must be a line in it. |
| `scripts/fetch_icons_multicloud.sh` | Azure, Google Cloud, Databricks, Snowflake and the data-platform marks. Refreshes `examples/icons/{azure,gcp,databricks,snowflake}/` and the root marks. **Azure is pinned to a pack version in the URL** (V24, 714 icons); Microsoft ships a new one every few months and the old link keeps working, so probe `V25`, `V26`… and pass the newest as `$1`. Checked 2026-09-15. |
...
| Azure | Azure Architecture Icons, https://learn.microsoft.com/azure/architecture/icons/ | **32 shipped in `examples/icons/azure/`.** Official package, ~670 SVGs named `NNNNN-icon-service-<Name>.svg`; the number is a release-specific prefix, so `fetch_icons_multicloud.sh` matches on the `<Name>` part. Note Microsoft's renames: Active Directory is now **Entra**, Data Factory is `Data-Factories`, Service Bus is `Azure-Service-Bus`. Simple Icons carries **no** Microsoft marks (trademark policy), so there is no Azure brand glyph — use a service icon |
| Google Cloud | Google Cloud architecture icons, https://cloud.google.com/icons | **41 shipped in `examples/icons/gcp/`.** Google gates its own zip behind a click-through, so these come from the diagrams project's mirror of the official set — PNG, transparent, ~1000px, so keep them at 2x like Loki and Tempo. `googlecloud.svg` (root) is the brand mark. Use Google's product names verbatim |
...
| Data platform & SaaS (Snowflake, Databricks, Airflow, Spark, Kafka, Flink, Hadoop, Looker, Kubernetes, Terraform, MongoDB, Redis, Elasticsearch, ClickHouse, DuckDB, Neo4j, MySQL) | **Shipped at the root of `examples/icons/`.** Simple Icons (`https://cdn.simpleicons.org/<slug>`); Apache projects use the `apache` prefix (`apacheairflow`, `apachekafka`, `apachespark`, `apacheflink`, `apachehadoop`) | Monochrome, so they take the figure's text colour. Respect vendor colour and clear-space rules when a figure uses brand colour |
| **Databricks** | **23 shipped in `examples/icons/databricks/`** | Databricks publishes no icon deck. These come from `oieduardorabelo/databricks-architecture-icons` (MIT), which packages Databricks' *own* SVGs — its `icons/sources/MANIFEST.md` gives a `www.databricks.com` origin URL per file, and spot-checking those URLs returns 200. 71 products available; the fetch list takes the ones a platform diagram actually needs |
| **Snowflake** | **21 shipped in `examples/icons/snowflake/`** | Snowflake's brand page ships icons only inside a **PowerPoint template** — they are PPT vector shapes, so nothing extractable (the .potx holds 44 PNGs, none of them the icons). These come from `sfc-gh-jcrittenden/snowflake-icons`, a Snowflake employee's repo (`sfc-gh-` is their staff prefix) holding 285 files in Snowflake's own `Snowflake_ICON_*` naming and brand blue `#29b5e8`. ⚠️ **No licence file.** Genuine artwork, but unlicensed distribution: fine for internal diagrams, get clearance before publishing externally. Only ~13 are true product icons (Snowpark, Cortex, Horizon, Iceberg/Dynamic Tables, the Warehouse family); the rest are concept glyphs |
| **SAP** | Corporate mark only (`sap.svg`, Simple Icons) | **SAP publishes no architecture icon set.** Fiori is a UI icon font, not service icons. Draw SAP systems (S/4HANA, BW, Datasphere, BTP) as labelled boxes carrying the SAP mark — never invent a per-product glyph. The same rule applies to any vendor without a published set |
